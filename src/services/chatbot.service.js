...
function normalizeColombiaPhone(rawPhone) {
  if (!rawPhone) return '';
  const digits = String(rawPhone).replace(/\D/g, '');

  if (!digits) return '';

  // Si viene con prefijo 57 y trae digitos extra, dejamos solo los ultimos 10
  if (digits.startsWith('57')) {
    const local = digits.slice(2);
    const normalizedLocal = local.length > 10 ? local.slice(-10) : local;
    return `+57${normalizedLocal}`;
  }

  // Si viene sin prefijo, asumimos numero local y dejamos solo los ultimos 10
  const normalizedLocal = digits.length > 10 ? digits.slice(-10) : digits;
  return `+57${normalizedLocal}`;
}

function isValidColombiaMobile(phone) {
...
async function gatherCandidateData(candidatoId) {
  ...
  return {
    ...
    telefono: normalizeColombiaPhone(candidate.telefono),
...
async function sendToChatbot(payload) {
  try {
    if (!isValidColombiaMobile(payload.telefono)) {
      const message = `Telefono invalido para candidato ${payload.candidato_id}: ${payload.telefono || 'VACIO'}`;
      console.log(`❌ [ChatbotService] ${message}`);
      console.error(`[ChatbotService] ${message}`);
      return {
        error_envio: true,
        message: 'Telefono invalido, no se envio al chatbot',
        details: message
      };
    }
